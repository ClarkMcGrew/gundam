...
" > $WORKDIR/inputs/fgd1fgd2Fit/toysReg/config_fit3_reg_toy${N}.json

	# Write a file containing the job
	echo -e "cd $WORKDIR; source setup.sh; xsllhFit -j $WORKDIR/inputs/fgd1fgd2Fit/toysReg/config_fit3_reg_toy${N}.json" > job_xsllh_fit3_reg_toy${N}.sh

	# Submit job
	qsub -l os=cl7,sps=1 -pe openmpi 16 -q pa_long job_xsllh_fit3_reg_toy${N}.sh
