...
//
//
//  Usage: root -b -q 'DrawEventComparison_onlyCC1pi.C+()'
//
///////////////////////////////////////////////////////////////////////////////////////
...
void DrawEventComparison_onlyCC1pi(string inputname = "fit2_dataCS_fakedataSignal_onlyCC1pi", const std::string& dir_name = "data", string fbinning = "/sps/t2k/lmaret/softwares/xsLLhFitterLM/inputs/fgd1fgd2Fit/binning/tn337_binning_GeV_format.txt")
{
	...
	for(int ifgd = 0; ifgd < Nfgd; ifgd++)
	{
		...
		for(int is=0; is<Nsample; is++)
		{
			...
			hSample_prefit[ifgd][is]  -> SetLineColor(kRed+1);
			hSample_postfit[ifgd][is] -> SetLineColor(kBlue+1);

			hSample_data[ifgd][is]    -> SetMarkerColor(kBlack);
			hSample_prefit[ifgd][is]  -> SetMarkerColor(kRed+1);
			hSample_postfit[ifgd][is] -> SetMarkerColor(kBlue+1);
			...
			TText *t1=paveFGD->AddText(Form("''#chi^{2}'' = %d", (int)calculateChi2(hSample_data[ifgd][is], hSample_postfit[ifgd][is]) ));
			paveFGD -> Draw();
	...
	for(int is=0; is<Nsample; is++)
	{
		...
		hSample_prefit_allSubDet[is]  -> SetLineColor(kRed+1);
		hSample_postfit_allSubDet[is] -> SetLineColor(kBlue+1);

		hSample_data_allSubDet[is]    -> SetMarkerColor(kBlack);
		hSample_prefit_allSubDet[is]  -> SetMarkerColor(kRed+1);
		hSample_postfit_allSubDet[is] -> SetMarkerColor(kBlue+1);
	...
	TCanvas* c_events_allSubDet_grouped = new TCanvas("NeventsGrouped","NeventsGrouped",1700,700);
	c_events_allSubDet_grouped -> Divide(3,2);
	...
	c_events_allSubDet_grouped->cd(6);
