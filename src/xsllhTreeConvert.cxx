...
int main(int argc, char** argv)
{
    ...
    // Pion variables:
    out_seltree -> Branch("D3True", &D3True, "D3True/F");
    out_seltree -> Branch("D3Reco", &D3Reco, "D3Reco/F");
    out_seltree -> Branch("D4True", &D4True, "D4True/F");
    out_seltree -> Branch("D4Reco", &D4Reco, "D4Reco/F");
    ...
    // Pion variables:
    out_trutree -> Branch("D3True", &D3True, "D3True/F");
    out_trutree -> Branch("D4True", &D4True, "D4True/F");
    ...
    for(const auto& file : v_files)
    {
        ...
        // Get Pion variables:
        TTreeReader reader_seltree("default", hl2_file);
        TTreeReaderArray<float> D3Reco_sel_array(reader_seltree, "selpi_mom");
        TTreeReaderArray<float> D4Reco_sel_array(reader_seltree, "selpi_costheta");
        TTreeReaderArray<float> D3True_sel_array(reader_seltree, "TruePosPionMom");
        TTreeReaderArray<float> D4True_sel_array(reader_seltree, "TruePosPionCosTheta");
        
        /*if(beammode == 1){
            TTreeReaderArray<float> D3Reco_sel_array(reader_seltree, "PosPionMom");
            TTreeReaderArray<float> D4Reco_sel_array(reader_seltree, "PosPionCosTheta");
            TTreeReaderArray<float> D3True_sel_array(reader_seltree, "TruePosPionMom");
            TTreeReaderArray<float> D4True_sel_array(reader_seltree, "TruePosPionCosTheta");
        }
        
        if(beammode == -1){
            TTreeReaderArray<float> D3Reco_sel_array(reader_seltree, "NegPionMom");
            TTreeReaderArray<float> D4Reco_sel_array(reader_seltree, "NegPionCosTheta");
            TTreeReaderArray<float> D3True_sel_array(reader_seltree, "TrueNegPionMom");
            TTreeReaderArray<float> D4True_sel_array(reader_seltree, "TrueNegPionCosTheta");
        }*/
        ...
        for(int i = 0; i < nevents; ++i)
        {
            ...
            for(const auto& kv : file.samples)
            {
                ...
                for(const auto& branch : kv.second)
                {
                    ...
                    {
                        // Here we set the pion variables. If there is exactly 1 reco pion and 1 true pion, we use the corresponding values:
                        if(D3Reco_sel_array.GetSize() == 1 && D3True_sel_array.GetSize() == 1 && D4Reco_sel_array.GetSize() == 1 && D4True_sel_array.GetSize() == 1)
                        {
                            D3Reco = D3Reco_sel_array[0];
                            D3True = D3True_sel_array[0];
                            D4Reco = D4Reco_sel_array[0];
                            D4True = D4True_sel_array[0];
                        }

                        // If we have 0 pions or more than one, we set all the pion variables to -999:
                        else
                        {
                            D3Reco = -999;
                            D3True = -999;
        ...
        // Get Pion variables:
        TTreeReader reader_trutree("truth", hl2_file);
        TTreeReaderArray<float> D3True_tru_array(reader_trutree, "TruePosPionMom");
        TTreeReaderArray<float> D4True_tru_array(reader_trutree, "TruePosPionCosTheta");
        ...
        for(int i = 0; i < nevents; ++i)
        {
            ...
            // Here we set the pion variables. If there is exactly 1 true pion, we use the corresponding values:
            if(D3True_tru_array.GetSize() == 1 && D4True_tru_array.GetSize() == 1)
            {
                D3True = D3True_tru_array[0];
                D4True = D4True_tru_array[0];
            }
